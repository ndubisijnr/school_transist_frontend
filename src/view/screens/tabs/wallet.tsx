...
//         // Refresh data
//         if(userDetails?.hub?.id){
//            const response =  await dispatch(app.action.readRideByHub(userDetails?.hub?.id))
...
//
//         if(userDetails?.student?.id){
//             const response = await dispatch(app.action.readRideByStudent(userDetails?.student?.id))
//             if(response.payload.code === "00") {
//
//             }else{
//                 console.error(response.payload);
...
const WalletScreen = () => {
    ...
    const fetchRidesData = useCallback(async (showLoading = false) => {
        if (showLoading) {
        ...
        try {
            ...
            if (userDetails?.hub?.id) {
                promises.push(dispatch(app.action.readRideByHub(userDetails.hub.id)));
            }

            if (userDetails?.student?.id) {
                promises.push(dispatch(app.action.readRideByStudent(userDetails.student.id)));
            }
    ...
    // Initial data fetch
    useEffect(() => {
        if (userDetails?.hub?.id || userDetails?.student?.id) {
            fetchRidesData(true);
        }
    }, [fetchRidesData]);

    // Focus effect - refreshes data when screen comes into focus
    useFocusEffect(
        useCallback(() => {
            // Only fetch if user has valid credentials
            if (userDetails?.hub?.id || userDetails?.student?.id) {
                fetchRidesData(false);
            }
        }, [fetchRidesData])
    );

    // Update combined rides when activities change
    useEffect(() => {
        // Combine rides from both student and hub activities
        const combinedRides = [
            ...(studentActivities || []),
    ...
    const getCurrentPageRides = useCallback(() => {
        const filteredRides = getFilteredRides();
        const startIndex = 0;
        const endIndex = currentPage * ITEMS_PER_PAGE;
        return filteredRides.slice(startIndex, endIndex);
    }, [getFilteredRides, currentPage]);
    ...
    }, [getFilteredRides, getCurrentPageRides, isLoadingMore]);
            ...
            if (isInitialLoading) {
                return (
                    <View className="flex-1 items-center justify-center">
                        ...
                        <Text className="mt-2 text-sm text-gray-500">Loading rides...</Text>
                    </View>
                );
            }

...
//     const transitStatusRef = useRef(currentRide?.transit_status);
...
//
//
//     // Keep the ref updated with latest status
//     useEffect(() => {
//         transitStatusRef.current = currentRide?.transit_status;
//     }, [currentRide?.transit_status]);
//
...
//         };
//
//         intervalRef.current = setInterval(pollWithCheck, 5000);
//
//         timeoutRef.current = setTimeout(() => {
//             if (!isMounted) return;
//
//             // Cleanup and state updates only if still mounted
...
//         };
//     }, [shouldPoll]); // Remove 'poll' if it's stable
...
const RideSearch = () => {
    ...
    const shouldPoll = currentRide?.transit_status === 'requested';
